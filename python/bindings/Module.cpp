...
PYBIND11_MODULE( TracyClientBindings, m )
{
    ...
    m.def(
        ...
        {
            if( !tracy::IsEnabled() ) return true;
            if( message.size() >= std::numeric_limits<uint16_t>::max() ) return false;
    ...
    m.def(
        "frame_image",
        []( const py::bytes& image, uint16_t width, uint16_t height, uint8_t offset = 0, bool flip = false )
        {
            if( !tracy::IsEnabled() ) return true;
            if( width % 4 != 0 || height % 4 != 0 ) return false;
            TracyCFrameImage( std::string( image ).data(), width, height, offset, flip );
            return true;
        },
        "data"_a.none( false ), "width"_a.none( false ), "height"_a.none( false ), "offset"_a.none( false ) = 0,
        "flip"_a.none( false ) = false );

    py::class_<PyScopedZone, std::shared_ptr<PyScopedZone>>( m, "_ScopedZone" )
        .def( py::init<const OptionalString&, uint32_t, OptionalInt, bool, const std::string&, const std::string&,
                       uint32_t>(),
              "name"_a, "color"_a.none( false ), "depth"_a, "active"_a.none( false ), "function"_a.none( false ),
              "file"_a.none( false ), "line"_a.none( false ) )
    ...
    m.def( "with_full_callstack", []() { tracy::GetProfiler(); } );

    m.def( "alloc", &MemoryAllocate<>, "ptr"_a.none( false ), "size"_a.none( false ),
           "name"_a = static_cast<OptionalString>( std::nullopt ), "id"_a = static_cast<OptionalNumber>( std::nullopt ),
           "depth"_a = static_cast<OptionalInt>( std::nullopt ) );
    m.def( "alloc", &MemoryAllocate<py::object>, "object"_a.none( false ), "size"_a.none( false ),
           "name"_a = static_cast<OptionalString>( std::nullopt ), "id"_a = static_cast<OptionalNumber>( std::nullopt ),
           "depth"_a = static_cast<OptionalInt>( std::nullopt ) );

    m.def( "free", &MemoryFree<>, "ptr"_a.none( false ), "id"_a = static_cast<OptionalNumber>( std::nullopt ),
           "depth"_a = static_cast<OptionalInt>( std::nullopt ) );
    m.def( "free", &MemoryFree<py::object>, "object"_a.none( false ),
           "id"_a = static_cast<OptionalNumber>( std::nullopt ), "depth"_a = static_cast<OptionalInt>( std::nullopt ) );

    m.def(
        "_plot_config",
        []( const std::string& name, int type, bool step, bool fill, uint32_t color = 0 )
        {
            if( !tracy::IsEnabled() ) return static_cast<OptionalNumber>( 0ul );
            auto entry = NameBuffer::Add( name );
            if( !entry.first ) return static_cast<OptionalNumber>( std::nullopt );
        ...
        "name"_a.none( false ), "type"_a.none( false ), "step"_a.none( false ), "fill"_a.none( false ),
        "color"_a.none( false ) );
