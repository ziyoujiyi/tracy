...
extern "C" typedef BOOL( WINAPI* t_GetLogicalProcessorInformationEx )( LOGICAL_PROCESSOR_RELATIONSHIP,
...
static bool EnsureReadable( MappingInfo& mapping )
{
    ...
    if( mprotect( reinterpret_cast<void*>( mapping.start_address ), mapping.end_address - mapping.start_address,
                  prot ) == -1 )
    {
...
#    else
static bool EnsureReadable( uintptr_t address ) { return true; }
#    endif
...
#        if !defined TRACY_TIMER_QPC && !defined TRACY_TIMER_FALLBACK
    ...
    {
#            if defined _WIN32
        InitFailure( "CPU doesn't support invariant TSC.\nDefine TRACY_NO_INVARIANT_CHECK=1 to ignore "
                     "this error, *if you know what you are doing*.\nAlternatively you may rebuild the "
                     "application with the TRACY_TIMER_QPC or TRACY_TIMER_FALLBACK define to use lower "
                     "resolution timer." );
#            else
        InitFailure( "CPU doesn't support invariant TSC.\nDefine TRACY_NO_INVARIANT_CHECK=1 to ignore "
                     "this error, *if you know what you are doing*.\nAlternatively you may rebuild the "
                     "application with the TRACY_TIMER_FALLBACK define to use lower resolution timer." );
#            endif
    }
#        endif
...
#    else
static int64_t SetupHwTimer() { return Profiler::GetTime(); }
#    endif
...
#    if defined __i386 || defined _M_IX86 || defined __x86_64__ || defined _M_X64
    ...
    {
        CpuId( regs, i );
        memcpy( modelPtr, regs, sizeof( regs ) );
        modelPtr += sizeof( regs );
    }
...
#    elif defined __linux__ && defined __ARM_ARCH
    ...
    if( fcpuinfo )
    {
        ...
        while( end - cptr > 20 )
        {
            ...
            else if( memcmp( cptr, "variant\t: ", 10 ) == 0 )
                var = GetHex( cptr, 10 );
            else if( memcmp( cptr, "part\t: ", 7 ) == 0 )
                part = GetHex( cptr, 7 );
            else if( memcmp( cptr, "revision\t: ", 11 ) == 0 )
                rev = GetHex( cptr, 11 );
            while( *cptr != '\n' && *cptr != '\0' ) cptr++;
            cptr++;
        }
        ...
        {
            cpuFound = true;
            ptr += sprintf( ptr, "CPU: %s%s r%ip%i\n", DecodeArmImplementer( impl ), DecodeArmPart( impl, part ), var,
                            rev );
        }
    }
    if( !cpuFound )
...
LONG WINAPI CrashFilter( PEXCEPTION_POINTERS pExp )
{
    ...
    case EXCEPTION_ACCESS_VIOLATION:
        ...
        {
        case 0:
            msgPtr += sprintf( msgPtr, "Read violation at address 0x%" PRIxPTR ".",
                               pExp->ExceptionRecord->ExceptionInformation[1] );
            break;
        case 1:
            msgPtr += sprintf( msgPtr, "Write violation at address 0x%" PRIxPTR ".",
                               pExp->ExceptionRecord->ExceptionInformation[1] );
            break;
        case 8:
            msgPtr += sprintf( msgPtr, "DEP violation at address 0x%" PRIxPTR ".",
                               pExp->ExceptionRecord->ExceptionInformation[1] );
            break;
        default:
            break;
    ...
    do
    {
        if( te.th32OwnerProcessID == pid && te.th32ThreadID != tid && te.th32ThreadID != s_profilerThreadId &&
            te.th32ThreadID != s_symbolThreadId )
        {
    ...
    while( !GetProfiler().HasShutdownFinished() )
    {
...
static Thread* s_pyTraceThread = nullptr;
#    endif
...
#        ifdef BUS_MCEERR_AO
        case BUS_MCEERR_AO:
            strcpy( msgPtr, "Hardware memory error detected in process but not consumed; action "
                            "optional.\n" );
            break;
#        endif
        default:
...
struct ProducerWrapper
{
    ProducerWrapper( ProfilerData& data )
        : detail( data.queue )
        , ptr( data.queue.get_explicit_producer( detail ) )
    {
    }
...
{
    ProfilerThreadData( ProfilerData& data )
        : token( data )
        , gpuCtx( { nullptr } )
    {
    }
...
TRACY_API bool IsProfilerStarted() { return s_isProfilerStarted.load( std::memory_order_seq_cst ); }
#        else
...
static ProfilerData& GetProfilerData()
{
    ...
    {
        int expected = 0;
        while( !profilerDataLock.compare_exchange_weak( expected, 1, std::memory_order_release,
...
// GCC prior to 8.4 had a bug with function-inline thread_local variables. Versions of glibc beginning
// with 2.18 may attempt to work around this issue, which manifests as a crash while running static
// destructors if this function is compiled into a shared object. Unfortunately, centos7 ships with
// glibc 2.17. If running on old GCC, use the old-fashioned way as a workaround See:
...
{
    return GetProfilerThreadData().token.ptr;
}
...
Profiler::Profiler()
    ...
    , m_epoch( std::chrono::duration_cast<std::chrono::seconds>( std::chrono::system_clock::now().time_since_epoch() )
                   .count() )
    , m_shutdown( false )
...
#    ifndef TRACY_DELAYED_INIT
#        ifdef _MSC_VER
    // 3. But these variables need to be initialized in main thread within the .CRT$XCB section. Do it
    // here.
...
#    ifdef TRACY_PY_TRACING
    ...
    s_pyTraceThread = (Thread*)tracy_malloc( sizeof( Thread ) );
    new( s_pyTraceThread ) Thread( PyTraceWorker, nullptr );
    std::this_thread::sleep_for( std::chrono::milliseconds( 1 ) );
    MyDebug( "PyTraceWorker started ..." );
#    endif

#    ifdef TRACY_HAS_SYSTEM_TRACING
    // use TRACY_NO_SYS_TRACE=1 to force disabling sys tracing (even if available in the underlying
    // system) as it can have significant impact on the size of the traces
    ...
    else if( SysTraceStart( m_samplingPeriod ) )
    {
        ...
        std::this_thread::sleep_for( std::chrono::milliseconds( 1 ) );
        MyDebug( "SysTraceWorker started ..." );
    }
    MyDebug( "disableSystrace = %d", disableSystrace );
#    endif
...
#    ifdef TRACY_PY_TRACING
    if( s_pyTraceThread )
    {
        ...
        s_pyTraceThread->~Thread();
        tracy_free( s_pyTraceThread );
    }
#    endif
...
bool Profiler::ShouldExit() { return s_instance->m_shutdown.load( std::memory_order_relaxed ); }
...
#    endif
    ...
    MyDebug( "dataport: %d, broadcastport: %d", dataPort, broadcastPort );
    // Connections loop.
...
#    endif
            ...
            if( m_broadcast )
            {
                ...
                if( t - lastBroadcast > 3000000000 ) // 3s
                {
                    ...
                    {
                        broadcastMsg =
                            GetBroadcastMessage( m_programName, strlen( m_programName ), broadcastLen, dataPort );
                        m_programName = nullptr;
                    }
...
#    ifdef TRACY_HAS_CALLSTACK
    while( s_symbolThreadGone.load() == false )
    {
        YieldThread();
    }
#    endif
...
void Profiler::ClearQueues( moodycamel::ConsumerToken& token )
{
    for( ;; )
    {
        const auto sz = GetQueue().try_dequeue_bulk_single(
            token, []( const uint64_t& ) {},
            []( QueueItem* item, size_t sz )
            {
                assert( sz > 0 );
                while( sz-- > 0 ) FreeAssociatedMemory( *item++ );
            } );
        if( sz == 0 ) break;
    }
...
#    define ThreadCtxCheckSerial( _name )                                                                              \
        ...
        case ThreadCtxStatus::Changed:                                                                                 \
            assert( m_refTimeThread == 0 );                                                                            \
            refThread = 0;                                                                                             \
            break;                                                                                                     \
        case ThreadCtxStatus::ConnectionLost:                                                                          \
...
{
    const lz4sz_t lz4sz =
        LZ4_compress_fast_continue( (LZ4_stream_t*)m_stream, data, m_lz4Buf + sizeof( lz4sz_t ), (int)len, LZ4Size, 1 );
...
{
    assert( type == QueueType::StringData || type == QueueType::ThreadName || type == QueueType::PlotName ||
            type == QueueType::FrameName || type == QueueType::ExternalName || type == QueueType::ExternalThreadName ||
...
{
    assert( type == QueueType::FrameImageData || type == QueueType::SymbolCode || type == QueueType::SourceCode );
...
void Profiler::QueueSourceCodeQuery( uint32_t id )
{
    assert( m_exectime != 0 );
    assert( m_queryData );
    m_symbolQueue.emplace(
        SymbolQueueItem{ SymbolQueueItemType::SourceCode, uint64_t( m_queryData ), uint64_t( m_queryImage ), id } );
    m_queryData = nullptr;
    m_queryImage = nullptr;
}
...
{
    // MyDebug( "(in client)SymbolQueueItem type: %d", si.type ); // 0,1,4
    switch( si.type )
    {
...
bool Profiler::HandleServerQuery()
{
    ...
    // MyDebug( "(in client)payload type: %d", type );
    switch( type )
    {
...
#    else
    ...
    static const tracy::SourceLocationData __tracy_source_location{ nullptr, TracyFunction, TracyFile,
    ...
    {
        const auto sz =
            GetQueue().try_dequeue_bulk_single( token, []( const uint64_t& ) {}, []( QueueItem* item, size_t sz ) {} );
        assert( sz > 0 );
        left -= (int)sz;
    }
...
#            else
    t_GetLogicalProcessorInformationEx _GetLogicalProcessorInformationEx =
        (t_GetLogicalProcessorInformationEx)GetProcAddress( GetModuleHandleA( "kernel32.dll" ),
...
void Profiler::HandleSourceCodeQuery( char* data, char* image, uint32_t id )
{
    ...
    {
        struct stat st;
        if( fstat( fileno( f ), &st ) == 0 && (uint64_t)st.st_mtime < m_exectime &&
            st.st_size < ( TargetFrameSize - 16 ) )
        {
...
#    endif
    ...
    // MyDebug("ok: %d, m_sourceCallback: %d", ok, m_sourceCallback);
...
#    endif
    ...
    TRACY_API TracyCZoneCtx ___tracy_emit_zone_begin_callstack( const struct ___tracy_source_location_data* srcloc,
                                                                int depth, int active )
    {
        ___tracy_c_zone_context ctx;
#    ifdef TRACY_ON_DEMAND
        ctx.active = active && tracy::GetProfiler().IsConnected();
...
#    endif
    ...
    TRACY_API void ___tracy_emit_memory_alloc_callstack_named( const void* ptr, size_t size, int depth, int secure,
                                                               const char* name )
    {
    ...
    {
        tracy::Profiler::ConfigurePlot( name, tracy::PlotFormatType( type ), step, fill, color );
    }
    TRACY_API void ___tracy_emit_message( const char* txt, size_t size, int callstack )
    {
        tracy::Profiler::Message( txt, size, callstack );
    }
    TRACY_API void ___tracy_emit_messageL( const char* txt, int callstack )
    {
        tracy::Profiler::Message( txt, callstack );
    }
    TRACY_API void ___tracy_emit_messageC( const char* txt, size_t size, uint32_t color, int callstack )
    {
        tracy::Profiler::MessageColor( txt, size, color, callstack );
    }
    TRACY_API void ___tracy_emit_messageLC( const char* txt, uint32_t color, int callstack )
    {
        tracy::Profiler::MessageColor( txt, color, callstack );
    }
    TRACY_API void ___tracy_emit_message_appinfo( const char* txt, size_t size )
    {
        tracy::Profiler::MessageAppInfo( txt, size );
    }

    TRACY_API uint64_t ___tracy_alloc_srcloc( uint32_t line, const char* source, size_t sourceSz, const char* function,
                                              size_t functionSz, uint32_t color )
    {
    ...
    TRACY_API void
    ___tracy_emit_gpu_zone_begin_alloc_callstack( const struct ___tracy_gpu_zone_begin_callstack_data data )
    {
        tracy::GetProfiler().SendCallstack( data.depth );
...
#    endif
    ...
    TRACY_API void
    ___tracy_emit_gpu_zone_begin_callstack_serial( const struct ___tracy_gpu_zone_begin_callstack_data data )
    {
    ...
    TRACY_API void
    ___tracy_emit_gpu_zone_begin_alloc_callstack_serial( const struct ___tracy_gpu_zone_begin_callstack_data data )
    {
        auto item = tracy::Profiler::QueueSerialCallstack( tracy::Callstack( data.depth ) );
...
#    endif
    ...
    TRACY_API struct __tracy_lockable_context_data*
    ___tracy_announce_lockable_ctx( const struct ___tracy_source_location_data* srcloc )
    {
        struct __tracy_lockable_context_data* lockdata =
            (__tracy_lockable_context_data*)tracy::tracy_malloc( sizeof( __tracy_lockable_context_data ) );
...
#    endif
    ...
    TRACY_API void ___tracy_custom_name_lockable_ctx( struct __tracy_lockable_context_data* lockdata, const char* name,
                                                      size_t nameSz )
    {
...
#    endif
    ...
    TRACY_API int ___tracy_connected( void ) { return tracy::GetProfiler().IsConnected(); }
