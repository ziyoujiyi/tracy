...
#    define TracyLine                                                                                                  \
        TracyConcat(                                                                                                   \
            __LINE__,                                                                                                  \
            U ) // MSVC Edit and continue __LINE__ is non-constant. See
...
#    define ZoneTransient( varname, active )                                                                           \
        tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ),  \
                                   nullptr, 0, TRACY_CALLSTACK, active )
#    define ZoneTransientN( varname, name, active )                                                                    \
        tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ),  \
                                   name, strlen( name ), TRACY_CALLSTACK, active )
#    define ZoneTransientNC( varname, name, color, active )                                                            \
        tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ),  \
                                   name, strlen( name ), color, TRACY_CALLSTACK, active )
...
#    define LockMark( varname )                                                                                        \
        static constexpr tracy::SourceLocationData __tracy_lock_location_##__LINE__{                                   \
            nullptr, TracyFunction, TracyFile, (uint32_t)TracyLine, 0 };                                               \
        varname.Mark( &__tracy_lock_location_##__LINE__ )
...
#    define ZoneTransientS( varname, depth, active )                                                                   \
        tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ),  \
                                   nullptr, 0, depth, active )
#    define ZoneTransientNS( varname, name, depth, active )                                                            \
        tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ),  \
                                   name, strlen( name ), depth, active )
