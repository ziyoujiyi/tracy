...
TRACY_API uint64_t ___tracy_alloc_srcloc_name( uint32_t line, const char* source, size_t sourceSz, const char* function,
...
TRACY_API void
___tracy_emit_gpu_zone_begin_alloc_callstack_serial( const struct ___tracy_gpu_zone_begin_callstack_data );
...
#        define TracyCZone( ctx, active )                                                                              \
            static const struct ___tracy_source_location_data TracyConcat( __tracy_source_location, TracyLine ) = {    \
                NULL, __func__, TracyFile, (uint32_t)TracyLine, 0 };                                                   \
...
#        define TracyCZoneC( ctx, color, active )                                                                      \
            static const struct ___tracy_source_location_data TracyConcat( __tracy_source_location, TracyLine ) = {    \
                NULL, __func__, TracyFile, (uint32_t)TracyLine, color };                                               \
            TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack(                                                    \
                &TracyConcat( __tracy_source_location, TracyLine ), TRACY_CALLSTACK, active );
#        define TracyCZoneNC( ctx, name, color, active )                                                               \
            static const struct ___tracy_source_location_data TracyConcat( __tracy_source_location, TracyLine ) = {    \
                name, __func__, TracyFile, (uint32_t)TracyLine, color };                                               \
            TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack(                                                    \
                &TracyConcat( __tracy_source_location, TracyLine ), TRACY_CALLSTACK, active );
#    else
#        define TracyCZone( ctx, active )                                                                              \
...
#    pragma message( "TRACY_HAS_CALLSTACK is " STRINGIFY( TRACY_HAS_CALLSTACK ) )
#    pragma message( "TRACY_FIBERS is " STRINGIFY( TRACY_FIBERS ) )
#    pragma message( "TRACY_CALLSTACK is " STRINGIFY( TRACY_CALLSTACK ) )
#    pragma message( "TRACY_NO_CALLSTACK_INLINES is " STRINGIFY( TRACY_NO_CALLSTACK_INLINES ) )
#    pragma message( "TRACY_NO_SAMPLING is " STRINGIFY( TRACY_NO_SAMPLING ) )
#    pragma message( "TRACY_NO_SYSTEM_TRACING is " STRINGIFY( TRACY_NO_SYSTEM_TRACING ) )
#    pragma message( "TRACY_LIBUNWIND_BACKTRACE is " STRINGIFY( TRACY_LIBUNWIND_BACKTRACE ) )
#    pragma message( "TRACY_SYMBOL_OFFLINE_RESOLVE is " STRINGIFY( TRACY_SYMBOL_OFFLINE_RESOLVE ) )
#    pragma message( "TRACY_LIBBACKTRACE_ELF_DYNLOAD_SUPPORT is " STRINGIFY( TRACY_LIBBACKTRACE_ELF_DYNLOAD_SUPPORT ) )
#    pragma message( "TRACY_DEMANGLE is " STRINGIFY( TRACY_DEMANGLE ) )
#    pragma message( "TRACY_NO_STATISTICS is " STRINGIFY( TRACY_NO_STATISTICS ) )
