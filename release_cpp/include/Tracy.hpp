...
#        define ZoneNamedC( varname, color, active )                                                                   \
            static constexpr tracy::SourceLocationData TracyConcat( __tracy_source_location, TracyLine ){              \
                nullptr, TracyFunction, TracyFile, (uint32_t)TracyLine, color };                                       \
            tracy::ScopedZone varname( &TracyConcat( __tracy_source_location, TracyLine ), TRACY_CALLSTACK, active )
...
#        define ZoneTransient( varname, active )                                                                       \
            tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,                       \
                                       strlen( TracyFunction ), nullptr, 0, TRACY_CALLSTACK, active )
#        define ZoneTransientN( varname, name, active )                                                                \
            tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,                       \
                                       strlen( TracyFunction ), name, strlen( name ), TRACY_CALLSTACK, active )
#        define ZoneTransientNC( varname, name, color, active )                                                        \
            tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,                       \
                                       strlen( TracyFunction ), name, strlen( name ), color, TRACY_CALLSTACK, active )
#    else
#        define ZoneNamed( varname, active )                                                                           \
            static constexpr tracy::SourceLocationData TracyConcat( __tracy_source_location, TracyLine ){              \
                nullptr, TracyFunction, TracyFile, (uint32_t)TracyLine, 0 };                                           \
...
#        define ZoneTransient( varname, active )                                                                       \
            tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,                       \
                                       strlen( TracyFunction ), nullptr, 0, active )
#        define ZoneTransientN( varname, name, active )                                                                \
            tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,                       \
                                       strlen( TracyFunction ), name, strlen( name ), active )
#        define ZoneTransientNC( varname, name, color, active )                                                        \
            tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,                       \
                                       strlen( TracyFunction ), name, strlen( name ), color, active )
#    endif
...
#    define FrameImage( image, width, height, offset, flip )                                                           \
        tracy::Profiler::SendFrameImage( image, width, height, offset, flip )
...
#    define TracyLockable( type, varname )                                                                             \
        ...
        {                                                                                                              \
            ...
            {                                                                                                          \
                static constexpr tracy::SourceLocationData srcloc{ nullptr, #type " " #varname, TracyFile, TracyLine,  \
                                                                   0 };                                                \
                return &srcloc;                                                                                        \
            }()                                                                                                        \
        }
#    define TracyLockableN( type, varname, desc )                                                                      \
        ...
        {                                                                                                              \
            []() -> const tracy::SourceLocationData*                                                                   \
            {                                                                                                          \
                static constexpr tracy::SourceLocationData srcloc{ nullptr, desc, TracyFile, TracyLine, 0 };           \
                return &srcloc;                                                                                        \
            }()                                                                                                        \
        }
...
#    define LockMark( varname )                                                                                        \
        static constexpr tracy::SourceLocationData __tracy_lock_location_##varname{ nullptr, TracyFunction, TracyFile, \
...
#    define TracyPlotConfig( name, type, step, fill, color )                                                           \
        tracy::Profiler::ConfigurePlot( name, type, step, fill, color )
...
#        define TracyAllocN( ptr, size, name )                                                                         \
            tracy::Profiler::MemAllocCallstackNamed( ptr, size, TRACY_CALLSTACK, false, name )
...
#        define TracySecureAllocN( ptr, size, name )                                                                   \
            tracy::Profiler::MemAllocCallstackNamed( ptr, size, TRACY_CALLSTACK, true, name )
...
#        define ZoneTransientS( varname, depth, active )                                                               \
            tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,                       \
                                       strlen( TracyFunction ), nullptr, 0, depth, active )
#        define ZoneTransientNS( varname, name, depth, active )                                                        \
            tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,                       \
                                       strlen( TracyFunction ), name, strlen( name ), depth, active )
...
#        define TracyAllocNS( ptr, size, depth, name )                                                                 \
            tracy::Profiler::MemAllocCallstackNamed( ptr, size, depth, false, name )
...
#        define TracySecureAllocNS( ptr, size, depth, name )                                                           \
            tracy::Profiler::MemAllocCallstackNamed( ptr, size, depth, true, name )
